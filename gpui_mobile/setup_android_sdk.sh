...
if [ ! -x "$SDK_DIR/cmdline-tools/latest/bin/sdkmanager" ]; then
    if [ -d cmdline-tools-tmp ]; then
        mv cmdline-tools-tmp "cmdline-tools-tmp.broken-$(date +%Y%m%d%H%M%S)"
    fi

    if [ -f "$CMDLINE_ZIP" ] && ! unzip -tq "$CMDLINE_ZIP" > /dev/null 2>&1; then
        echo "⚠️  Android 命令行工具压缩包已损坏，重新下载..."
        mv "$CMDLINE_ZIP" "$CMDLINE_ZIP.broken-$(date +%Y%m%d%H%M%S)"
    fi
    ...
    if [ -d "$SDK_DIR/cmdline-tools/latest" ]; then
        mv "$SDK_DIR/cmdline-tools/latest" "$SDK_DIR/cmdline-tools/latest.broken-$(date +%Y%m%d%H%M%S)"
    fi
    mv cmdline-tools-tmp/cmdline-tools "$SDK_DIR/cmdline-tools/latest"
    if [ -d cmdline-tools-tmp ]; then
        mv cmdline-tools-tmp "cmdline-tools-tmp.done-$(date +%Y%m%d%H%M%S)"
    fi
    if [ -f "$CMDLINE_ZIP" ]; then
        mv "$CMDLINE_ZIP" "$CMDLINE_ZIP.done-$(date +%Y%m%d%H%M%S)"
    fi
else
...
echo "   3. 构建项目: cd /home/zdz/Documents/Try/Rust/learning/mobile && ./build.sh"
echo ""
echo "📱 连接 Android 设备后："
