...
手机也可以作为服务端：打开 App 首页的「作为服务器」，App 会在私有
`files/shared` 目录启动 HTTP 服务并显示地址和二维码。PC 浏览器访问该地址后，
可以直接上传或下载文件。手机端列表中的「转移到下载」会把文件从私有共享目录
转移到公共 Downloads 目录；退出页面时服务停止。

手机作为 server 时，点击页面中的「添加手机文件」，通过系统文件选择器选择手机
文件后，文件会加入私有共享目录；PC 浏览器刷新列表后即可下载该文件。
